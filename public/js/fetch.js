...
if (page_myWorkassignment_content) {
  ...
  //ตารางหน้าประวัติการทำงานของฉัน
  ...
  axios.get("/main/myWorkHistory/data")
    .then(response => {
      const data = response.data; // axios จะใส่ JSON ไว้ใน response.data
 ...
 const page_home_Container = document.getElementById('page-main-content');
  if (page_home_Container) {
    ...
    // ดึง latest 3 problems มาแสดงใต้ชื่อผู้ใช้
  ...
  if (form) {
    try {
    // ตรวจสอบ session
    const sessionRes = await axios.get("/api/check-session", { withCredentials: true });
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const res = await axios.post("/add-problem", data, { withCredentials: true });
...
if (dropdown) {
  ...
  if (dropdowndep) {
    let loadeddep = false;
    dropdowndep.addEventListener("click", async () => {
      if (loadeddep) return;

      try {
        const res = await axios.get("/main/department");
        const data = res.data;
  ...
  // ดึง dropdown ระดับความสำคัญ
  const dropdownpri = document.getElementById("priorityDropdown");
  if (dropdownpri) {
    let loadedpri = false;
    dropdownpri.addEventListener("click", async () => {
      if (loadedpri) return;

      try {
        const res = await axios.get("/main/priority");
        const data = res.data;
...
axios.get("/main/users/data")
  .then(res => {
    ...
    if (nameEl) {
      if (page_home_Container) {
        nameEl.textContent = user.firstname || "ไม่ระบุชื่อ"; // หน้า Home เติมชื่อผู้ใช้
      } else if (page_history_Container) {
        nameEl.textContent = "";  // หน้า My History เว้นว่าง
      }
    }
